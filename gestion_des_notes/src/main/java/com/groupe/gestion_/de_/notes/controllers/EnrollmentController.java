...
public class EnrollmentController {
    ...
    @Operation(summary = "Get enrollments by class", description = "Retrieve all enrollments for a specific class.")
    @ApiResponses(value = {
            @ApiResponse(responseCode = "200", description = "Enrollments retrieved successfully"),
            @ApiResponse(responseCode = "403", description = "Forbidden: Insufficient privileges"),
    ...
    @Operation(summary = "Update an enrollment", description = "Updates a specific enrollment record. Only Admins can perform this action.")
    @ApiResponses(value = {
            @ApiResponse(responseCode = "200", description = "Enrollment updated successfully"),
            @ApiResponse(responseCode = "400", description = "Invalid input or student is already enrolled in the new configuration"),
            @ApiResponse(responseCode = "403", description = "Forbidden: Only Admins can update enrollments"),
            @ApiResponse(responseCode = "404", description = "Enrollment, Student, Class, or Subject not found")
    })
    @PutMapping("/{id}")
    @PreAuthorize("hasRole('ADMIN')")
