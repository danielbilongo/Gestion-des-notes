...
public class SubjectController {
    ...
    public ResponseEntity<SubjectResponse> findBySubjectCode(@PathVariable String subjectCode) {
        return subjectService.findBySubjectCode(subjectCode)
                ...
                .orElseThrow(() -> new NoSuchElementException("Subject not found with code: " + subjectCode));
    }
    ...
    @Operation(summary = "Fix semesters for existing subjects", description = "Temporary endpoint to fix semester data")
    @PostMapping("/fix-semesters")
    @PreAuthorize("hasRole('ADMIN')")
    public ResponseEntity<String> fixSemesters() {
        subjectService.fixSemesters();
        return ResponseEntity.ok("Semesters fixed successfully");
    }
}
