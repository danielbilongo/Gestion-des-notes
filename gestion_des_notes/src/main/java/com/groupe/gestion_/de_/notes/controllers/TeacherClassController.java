...
public class TeacherClassController {
     ...
     * Accessible by ADMINs and TEACHERs.
     */
    @Operation(summary = "Get all teacher-class assignments", description = "Retrieve all teacher-class assignments in the system.")
    @ApiResponses(value = {
            @ApiResponse(responseCode = "200", description = "Assignments retrieved successfully"),
            @ApiResponse(responseCode = "403", description = "Forbidden: Only Admins and Teachers can perform this action")
    })
    @GetMapping
    @PreAuthorize("hasAnyRole('ADMIN', 'TEACHER')")
    public ResponseEntity<List<TeacherClassResponse>> getAllTeacherClassAssignments() {
        List<TeacherClassResponse> assignments = teacherClassService.getAllTeacherClassAssignments();
     ...
     * This operation is restricted to ADMINs only.
     */
    @Operation(summary = "Update a teacher-class assignment", description = "Updates an existing assignment by changing teacher or class.")
    @ApiResponses(value = {
            @ApiResponse(responseCode = "200", description = "Assignment updated successfully"),
            @ApiResponse(responseCode = "400", description = "Invalid input or assignment already exists"),
            @ApiResponse(responseCode = "403", description = "Forbidden: Only Admins can perform this action"),
            @ApiResponse(responseCode = "404", description = "Assignment, Teacher, or Class not found")
    })
    @PutMapping("/{id}")
    @PreAuthorize("hasRole('ADMIN')")
    public ResponseEntity<TeacherClassResponse> updateTeacherClassAssignment(@PathVariable Long id, @Valid @RequestBody TeacherClassRequest request) {
        TeacherClassResponse response = teacherClassService.updateTeacherClassAssignment(id, request);
     ...
     * This operation is restricted to ADMINs only.
     */
    @Operation(summary = "Delete a teacher-class assignment", description = "Deletes an assignment by its unique ID.")
    @ApiResponses(value = {
    ...
    @DeleteMapping("/{id}")
    @PreAuthorize("hasRole('ADMIN')")
    public ResponseEntity<Void> deleteTeacherClassAssignment(@PathVariable Long id) {
        teacherClassService.deleteTeacherClassAssignment(id);
