...
public class UserController {
    ...
    public ResponseEntity<Optional<UserResponse>> findByUsername(@Valid @PathVariable String username){
        ...
        if(optionalresponse.isPresent()){
            optionalresponse.get();
            return new ResponseEntity<>(optionalresponse, HttpStatus.OK);
        }else {
            return new ResponseEntity<>(optionalresponse, HttpStatus.NOT_FOUND);
        }
    }
    ...
    @PreAuthorize("hasRole('ADMIN')") // ONLY ADMIN can access all users
    public ResponseEntity<List<UserResponse>> getAllUsers(){
        List<UserResponse> listuser = userService.getAllUsers();
        return new ResponseEntity<>(listuser, HttpStatus.OK);
    }
    @Operation(summary = "endpoint to get all students with studentIdNum!")
    @GetMapping("/students/all") // New endpoint for students
    @PreAuthorize("hasRole('ADMIN') or hasRole('TEACHER')") // ADMIN and TEACHER can access students for grade management
    public ResponseEntity<List<StudentResponse>> getAllStudents(){
        List<StudentResponse> students = userService.getAllStudents();
        return new ResponseEntity<>(students, HttpStatus.OK);
    }

    @Operation(summary = "endpoint to get all teachers with teacherIdNum!")
    @GetMapping("/teachers/all") // New endpoint for teachers
    @PreAuthorize("hasRole('ADMIN')") // ONLY ADMIN can make researches on users
    ...
    @Operation(summary = "endpoint to get students enrolled in a specific subject!")
    @GetMapping("/students/by-subject/{subjectCode}") // New endpoint for students by subject
    @PreAuthorize("hasRole('ADMIN') or hasRole('TEACHER')") // ADMIN and TEACHER can access students for grade management
    public ResponseEntity<List<StudentResponse>> getStudentsBySubject(@PathVariable String subjectCode){
        List<StudentResponse> students = userService.getStudentsBySubject(subjectCode);
        return new ResponseEntity<>(students, HttpStatus.OK);
    }
