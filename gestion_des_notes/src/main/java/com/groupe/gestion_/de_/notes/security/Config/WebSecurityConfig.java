...
import com.groupe.gestion_.de_.notes.security.Jwt.CustomAuthenticationSuccessHandler;
...
public class WebSecurityConfig {
    ...
    public AuthTokenFilter authenticationJwtTokenFilter() {
        return new AuthTokenFilter(jwtUtils, userDetailsServiceImpl, authenticationSuccessHandler);
    }
    ...
    public CorsConfigurationSource corsConfigurationSource() {
        ...
        // Allow your React frontend's origin
        configuration.setAllowedOrigins(List.of("http://localhost:4300")); // React frontend URL
    ...
    public SecurityFilterChain filterChain(HttpSecurity http) throws Exception {
        http.csrf(AbstractHttpConfigurer::disable) // Disable CSRF for Stateless REST APIs
                ...
                .authorizeHttpRequests(authorize -> authorize
                        ...
                        // Specific endpoints that teachers need access to (BEFORE general /api/admin/** rule)
                        .requestMatchers("/api/admin/users/students/all").hasAnyRole("ADMIN", "TEACHER")
                        .requestMatchers("/api/admin/users/students/by-subject/**").hasAnyRole("ADMIN", "TEACHER")
                        // API endpoints accessible to multiple roles
                        ...
                        .requestMatchers("/api/classes/**").hasAnyRole("ADMIN", "TEACHER", "STUDENT")
                        .requestMatchers("/api/teacher-classes/**").hasAnyRole("ADMIN", "TEACHER")
                        .requestMatchers("/api/transcript/**").hasAnyRole("ADMIN", "TEACHER", "STUDENT")
