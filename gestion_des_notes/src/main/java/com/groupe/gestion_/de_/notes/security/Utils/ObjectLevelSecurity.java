...
ObjectLevelSecurity {
     ...
     *
     * @param subjectCode The code of the subject to check (e.g., "MATH101").
    ...
    public boolean isTeacherAssignedToSubject(String subjectCode) {
        ...
        // 1. Get the teacher by user ID to get teacherIdNum
        ...
        // 2. Get all classes the teacher is assigned to
        List<TeacherClass> teacherClasses = teacherClassRepository.findByTeacher_TeacherIdNum(teacherIdNum);
        if (teacherClasses.isEmpty()) {
            return false;
        }

        // 3. Get all subjects offered in those classes
        Set<String> subjectCodesTaughtByTeacher = new HashSet<>();
        for (TeacherClass tc : teacherClasses) {
            ...
            subjectCodesTaughtByTeacher.addAll(classSubjects.stream()
                    .map(cs -> cs.getSubject().getSubjectCode())
                    .collect(Collectors.toSet()));
        }

        // 4. Check if the target subjectCode is among the subjects the teacher teaches
        return subjectCodesTaughtByTeacher.contains(subjectCode);
    }
